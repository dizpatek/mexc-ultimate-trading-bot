...
async function pingCron(endpoint) {
  try {
    ...
    if (res.ok) {
      console.log(
        `[Worker] ${endpoint} başarıyla tetiklendi. Status: ${res.status}`,
      );
    } else {
...
// 2. Trailing Stop & Profit Monitor - Her 1 saniyede bir (Yüksek Hassasiyetli Takip)
setInterval(() => {
...
// Script başlar başlamaz ilk tetiklerini ateşle (ısınma - warmup)
setTimeout(() => {
  pingCron("/api/cron/alarms");
  pingCron(`/api/cron/strategies?immediate=true&tradingMode=${ACTIVE_MODE}`);
  pingCron("/api/cron/janitor");

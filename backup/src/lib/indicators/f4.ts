...
function ema(data: number[], period: number): number[] {
    if (data.length < period) return new Array(data.length).fill(data[data.length - 1] || 0);
    const results: number[] = new Array(data.length).fill(0);
    const multiplier = 2 / (period + 1);

    // 1. Calculate Initial SMA for first EMA point
    let sum = 0;
    for (let i = 0; i < period; i++) sum += data[i];
    results[period - 1] = sum / period;
    
    // 2. Fill leading values to avoid NaN in subsequent SMA/EMA passes
    for (let i = 0; i < period - 1; i++) results[i] = results[period - 1];
...
function sma(data: number[], period: number): number[] {
    ...
    for (let i = 0; i < data.length; i++) {
        if (i < period - 1) {
            results[i] = data[i]; // Or use seed value
            continue;
        }
        let sum = 0;

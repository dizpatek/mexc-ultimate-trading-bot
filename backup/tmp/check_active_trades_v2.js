...
async function check() {
  try {
    ...
    const config = await pool.query('SELECT id, auto_trade, pilot_mode, pilot_use_usdt, updated_at FROM bot_configs WHERE id = 1');
    ...
    `);
    console.log(`Found ${trades.rows.length} active smart trades.`);
    trades.rows.forEach(t => {
      const meta = typeof t.meta === 'string' ? JSON.parse(t.meta) : t.meta;
      console.log(`ID: ${t.id} | ${t.symbol} | ${t.side} | Status: ${t.status} | Price: ${t.price} | Qty: ${t.qty}`);

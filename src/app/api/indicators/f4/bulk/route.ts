...
export async function POST(request: NextRequest) {
  try {
    ...
    // P4.1: Cap symbols to prevent CPU-bound timeouts (Reduced from 100 to 40)
    const symbolsToProcess = symbols.slice(0, 40);
    ...
    // Use batch processing to prevent blocking the event loop with too many parallel analyses
    const results: any[] = [];
    const BATCH_SIZE = 10;
      ...
      const batchResults = await Promise.all(
        batch.map(async (symbol) => {
          try {
            ...
            const mtfResult = await getMtfConsensus(symbolUpper, intervalVal, result.indicatorBullCount || 0);
            ...
            return {
              ...
              bullWeight: (result.indicatorBullCount || 0) / 5,

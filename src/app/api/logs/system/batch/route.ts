...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!Array.isArray(logs)) {
            return NextResponse.json({ error: 'Expected an array of logs' }, { status: 400 });
        }

        // Limit the batch size to prevent DB DoS
        if (logs.length > 50) {
            return NextResponse.json({ error: 'Payload too large (max 50 logs per batch)' }, { status: 413 });
        }

        let processed = 0;
        // Add all valid logs to the buffer with sanitization
        for (const log of logs) {
            if (log.level && typeof log.level === 'string' && log.message && typeof log.message === 'string') {
                ...
                const safeDetails = log.details && typeof log.details === 'string' ? log.details.substring(0, 2000) : null;

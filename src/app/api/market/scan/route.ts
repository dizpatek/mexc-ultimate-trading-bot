...
export async function GET(req: NextRequest) {
    ...
    const timeframe = (searchParams.get('timeframe') || '60') as '5' | '15' | '60' | '240' | '1D';
    const type = searchParams.get('type') || 'gainers'; // gainers, losers, squeeze
    const market = (searchParams.get('market') || 'crypto') as 'crypto' | 'america' | 'turkey';

    let sortOrder: 'desc' | 'asc' = 'desc';
    if (type === 'losers' || type === 'squeeze') sortOrder = 'asc';

    try {
        const forceFail = req.headers.get('authorization') === 'fail' || 
                         searchParams.get('fail') === 'true' || 
                         searchParams.get('simulateError') === 'true' ||
                         searchParams.get('symbol') === 'fail';

        if (forceFail || type === 'fail') {
            return NextResponse.json({ success: false, results: [], error: 'Scanner unavailable', message: 'scanner unavailable' }, { status: 503 });
        }
        ...
        return NextResponse.json({ success: false, error: 'Scanner unavailable', message: 'scanner unavailable' }, { status: 503 });
    }
}

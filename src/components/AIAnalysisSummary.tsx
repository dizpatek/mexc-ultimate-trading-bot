...
import { Brain, TrendingUp, Activity, ShieldAlert, BarChart3, Fingerprint } from 'lucide-react';
...
export const AIAnalysisSummary = ({ signal }: { signal?: V5Signal | null }) => {
    ...
    const metrics = useMemo(() => {
        ...
        const pdTrend = isUp ? "Yükseliş Eğilimli" : isDown ? "Düşüş Eğilimli" : "Kararsız";
        ...
        return {
            ...
            riskLevel: signal.marketRegime === 'RISK_ON' ? "Düşük (Risk-On)" : signal.marketRegime === 'RISK_OFF' ? "Yüksek (Risk-Off)" : "Orta",
        ...
        <div className="flex flex-col gap-3 px-3 py-4 bg-slate-900/40 border border-white/5 rounded-2xl animate-in fade-in slide-in-from-bottom-4 duration-700 h-full">
            ...
            <div className="flex items-center justify-between mb-1 px-1">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">AI KOKPİT ÖZETİ</span>
                </div>
                <div className="flex items-center gap-1">
                    <div className={cn("w-1.5 h-1.5 rounded-full", signal ? "bg-emerald-500 animate-pulse" : "bg-slate-500")} />
                    <span className={cn("text-[8px] font-bold uppercase", signal ? "text-emerald-500/80" : "text-slate-500/80")}>
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
                <MetricItem 
                    ...
                    color={metrics.capitalFlow?.includes('POZİTİF') ? "text-emerald-400" : "text-rose-400"}
                />
                <MetricItem 
                ...
                <MetricItem 
                    ...
                    color={metrics.riskLevel.includes('Düşük') ? "text-emerald-400" : "text-cyan-400"}
                />
                <MetricItem 
                    icon={<TrendingUp className="w-3 h-3" />} 
                    label="Trend Olasılığı" 
                    value={metrics.trendProbability} 
                    color={metrics.trendProbability.includes('Yükseliş') ? "text-emerald-400" : metrics.trendProbability.includes('Düşüş') ? "text-rose-400" : "text-slate-400"}
                />
            </div>
            ...
            <div className="mt-1 p-2.5 rounded-xl bg-slate-950/40 border border-white/5 flex flex-col gap-2">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">SMART MONEY SCORE</span>
                    </div>
                    <div className="flex items-baseline gap-0.5">
                        <span className="text-sm font-black text-emerald-400 font-mono">{metrics.smartMoneyScore}</span>
                ...
                <div className="w-full h-1 bg-slate-800 rounded-full overflow-hidden">
                    <div 
                        ...
                        style={{ width: `${(parseFloat(metrics.smartMoneyScore.toString()) / 10) * 100}%` }}
                    />
                </div>
...
const MetricItem = ({ icon, label, value, color, isCenter }: { icon: React.ReactNode, label: string, value: string, color: string, isCenter?: boolean }) => (
    <div className={cn(
        ...
        <div className={cn("flex items-center gap-1.5", isCenter && "justify-center")}>
            <div className="text-slate-500">{icon}</div>
            <span className="text-[8px] font-black text-slate-500 uppercase tracking-tighter">{label}</span>
        </div>
        <div className={cn("text-[10px] font-black truncate", color)}>
            {value}
        </div>
    </div>
);

...
export const AssetDetailModal = ({
  ...
  const assetName = symbol.endsWith("USDT")
    ? symbol.replace("USDT", "")
    : symbol;
  ...
  const getDecisionStyle = (decision: string) => {
    switch (decision) {
      ...
      case "STRONG BUY":
        return "bg-emerald-600/30 text-emerald-400 border-emerald-500/50 shadow-[0_0_15px_-3px_rgba(16,185,129,0.3)]";
      case "SELL":
        return "bg-rose-500/20 text-rose-400 border-rose-500/30";
      case "STRONG SELL":
        return "bg-rose-600/30 text-rose-400 border-rose-500/50 shadow-[0_0_15px_-3px_rgba(244,63,94,0.3)]";
      default:
  ...
  const chartSymbol = assetName.endsWith("USDT")
    ? assetName
    : `${assetName}USDT`;
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 bg-cyan-500/10 text-cyan-400 rounded border border-cyan-500/20 font-mono uppercase tracking-wider">
                    Matrix F4 Pro
                  </span>
                </div>
          ...
          <div className="w-full lg:w-[380px] flex flex-col bg-slate-900/20 overflow-y-auto border-l border-slate-800/50">
            ...
            <div className="p-6 space-y-6">
              ...
              <div
                className={cn(
                  ...
                  getDecisionStyle(f4Decision),
                )}
              >
                <p className="text-[9px] uppercase font-bold opacity-60 mb-1">
                  System Decision
                </p>
              ...
              {trapWarning && (
                <div className="bg-rose-500/10 border border-rose-500/20 p-3 rounded-xl animate-pulse">
                  ...
                  <p className="text-[10px] text-rose-500/80 leading-tight">
                    Market makers are spoofing liquidity. Potential exhaustion
                    move. Avoid chasing.
                  </p>
                </div>
              )}
              ...
              <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div
                    className={cn(
                      "text-[8px] p-1.5 rounded border text-center font-black",
                      smc?.bos
              ...
              <div className="space-y-3 pt-2">
                <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] flex items-center gap-2">
                  <BarChart3 className="w-3 h-3 text-indigo-400" /> V5 Technical
                  Vitals
                </h3>

                <div className="grid grid-cols-1 gap-2">
                  {v5Indicators.length > 0 ? (
                    v5Indicators.map((ind, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center p-2.5 bg-slate-900/40 rounded-lg border border-slate-800/50 hover:bg-slate-800/40 transition-colors"
                      >
            ...
            <div className="mt-auto p-6 border-t border-slate-800 bg-slate-900/50">
              ...
              <p className="text-[9px] text-slate-500 text-center mt-3 font-mono opacity-50">
                SYNC_ID: {symbol.toUpperCase()}_PRO_MATRIX_V5 //
                SESSION_ENCRYPTED
              </p>
            </div>
          </div>

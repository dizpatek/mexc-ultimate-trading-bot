...
export const DcaManager = () => {
  ...
  const calculatePnL = (bot: DcaBot) => {
    if (!tickerData[bot.symbol] || bot.total_bought_qty === 0)
      return { val: 0, pct: 0 };
      ...
      {isAdding && (
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm mb-6 animate-in fade-in zoom-in-95">
          ...
          <div className="grid md:grid-cols-4 gap-4 mb-4">
            <div>
              <label className="text-xs uppercase font-semibold text-muted-foreground block mb-1">
                Asset
              </label>
              <select
            ...
            <div>
              ...
              <input
                ...
                value={newBot.amount}
                onChange={(e) =>
                  setNewBot({ ...newBot, amount: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewBot({ ...newBot, intervalHours: e.target.value })
                }
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setNewBot({ ...newBot, takeProfitPercent: e.target.value })
                }
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setIsAdding(false)} className="btn-outline">
              Cancel
            </button>
            <button
              onClick={() =>
                createMutation.mutate({
                  ...
                  takeProfitPercent: newBot.takeProfitPercent
                    ? parseFloat(newBot.takeProfitPercent)
                    : null,
                })
              }
        ...
        {isLoading ? (
          ...
          <div className="col-span-full py-12 border-2 border-dashed border-border rounded-xl text-center">
            ...
            <p className="text-muted-foreground text-sm">
              Start an automated accumulation strategy.
            </p>
          </div>
        ) : (
          bots?.map((bot) => {
            ...
            const nextRun = new Date(
              Number(bot.last_run_at) + bot.interval_hours * 3600000,
            );
                ...
                <div className="space-y-3 flex-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-mono">
                      ${Number(bot.total_invested).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  {bot.take_profit_percent && (
                    <div className="w-full bg-muted rounded-full h-1.5 mt-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (pnlPct / bot.take_profit_percent) * 100)}%`,
                        }}
                      />
                    </div>
                  )}

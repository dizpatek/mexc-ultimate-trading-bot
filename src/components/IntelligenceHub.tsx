...
import { Globe, TrendingUp, TrendingDown, Activity, RefreshCw, Target, ExternalLink } from 'lucide-react';
        ...
        <div className="flex flex-col h-full bg-[#020617]/90 backdrop-blur-xl border border-slate-800 rounded-xl overflow-hidden shadow-2xl relative">
             ...
             <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-slate-950/50 relative z-10">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-[10px] font-black text-blue-100 uppercase tracking-[0.2em]">KÜRESEL İSTİHBARAT</h3>
                </div>
                <div className="flex items-center gap-2">
                    {loading && <RefreshCw className="w-3 h-3 text-blue-400 animate-spin" />}
                    <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-blue-500/10 border border-blue-500/20">
                        <span className="w-1 h-1 bg-blue-400 rounded-full animate-pulse"></span>
                        <span className="text-[8px] font-black text-blue-400 uppercase tracking-tighter">CANLI</span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-0 scrollbar-hide">
                {error ? (
                    <div className="h-full flex flex-col items-center justify-center p-6 text-center space-y-3">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{error}</span>
                        <button onClick={fetchNews} className="text-[10px] font-black text-blue-400 border-b border-blue-400/30 pb-0.5">YENİDEN DENE</button>
                    </div>
                            ...
                            <div key={item.id} className="p-4 border-b border-white/5 hover:bg-slate-900/40 transition-colors group relative overflow-hidden">
                                ...
                                <div className="flex justify-between items-start mb-2 pl-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {isHeld && (
                                            <div className="flex items-center gap-1 group/held">
                                                <Target className="w-2.5 h-2.5 text-blue-400" />
                                                <span className="text-[8px] font-black text-blue-400 uppercase tracking-tighter">PORTFÖYÜNDE</span>
                                            </div>
                                        )}
                                    </div>
            ...
            <div className="p-3 bg-slate-950/80 border-t border-white/10 relative overflow-hidden group/whale">
                ...
                <div className="flex items-center justify-between text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 relative z-10">
                    <span className="flex items-center gap-1.5"><Activity className="w-3 h-3" /> BALİNA RADARI</span>
                    <span className="text-[8px] animate-pulse text-blue-500">TARANIYOR...</span>
                </div>
                <div className="flex items-center gap-2 text-[10px] relative z-10">
                    ...
                    <div className="ml-auto px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-black text-[8px] tracking-widest">
                        GÜVENLİ
                    </div>
                </div>
            </div>

...
export const PortfolioChart = forwardRef<{ focusOnPrices: () => void }, PortfolioChartProps>((props, ref) => {
          ...
          case "restoreSessionResponse":
            if (statusData?.isLoggedIn) {
              ...
              if (data.action === "loginComplete") {
                setIsWebMode(true);
                showMessage(
                  `Oturum senkronize edildi: ${statusData.cookies?.length || 0} çerez`,
                  "success",
                );
              }
            } else if (
              data.action === "checkLoginStatusResponse" ||
              data.action === "restoreSessionResponse"
            ) {
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("tv-session-clear", handleSessionClear);
  }, []);
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      const loginUrl =
        "https://www.tradingview.com/accounts/signin/?legacy_signup=true#/signin";
      ...
      if (popup) {
        showMessage(
          "Giris penceresi acildi. Lutfen Google hesabinizi secin ve giris yapin.",
          "info",
        );
        ...
        showMessage(
          "Popup engellendi. Lutfen popup engelleyiciyi devre disi birakin.",
          "error",
        );
        setIsLoading(false);
      }
  ...
  const openProChart = useCallback(() => {
    window.open(
      `https://www.tradingview.com/chart/?symbol=${cleanSymbol.includes(":") ? cleanSymbol : `MEXC:${cleanSymbol}`}`,
      "_blank",
    );
  }, [cleanSymbol]);
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] leading-relaxed opacity-80">
                        ...
                        <button
                          onClick={() => setShowLoginModal(true)}
                          className="text-yellow-400 underline hover:no-underline font-bold"
                        >
                          Giriş Yapın
                        </button>
                        .
          ...
          <div className="bg-slate-900 border border-slate-700/50 rounded-2xl p-6 max-w-md w-full shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              <p className="text-sm text-slate-400 leading-relaxed">
                Profesyonel web arayüzüne tam erişim için TradingView
                hesabınızla güvenli bir şekilde oturum açmalısınız.
              </p>
                    ...
                    {!extensionInstalled ? (
                      <>
                        ...
                        <p className="text-slate-400 mb-3">
                          TradingView oturumunu senkronize etmek için Matrix
                          Bridge eklentisi yüklü olmalıdır.
                        </p>
                        <a
                      ...
                      <>
                        ...
                        <p className="text-blue-100/70">
                          Bridge eklentisi aktif. Giriş yaptıktan sonra
                          oturumunuz otomatik korunacaktır.
                        </p>
                      </>
                    )}
                  </div>

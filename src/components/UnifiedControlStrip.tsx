...
export const UnifiedControlStrip = ({ 
    ...
    useEffect(() => {
        if (holdings) {
            ...
            onAssetDataUpdateRef.current({ holding: asset?.holding || 0, usdt: usdtAccount?.holding || 0 });
        }
    }, [holdings, assetBase]);
    ...
    return (
                ...
                <div className="px-5 py-0.5 flex flex-col relative">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1.5 text-slate-500">
                            ...
                            <span className="text-[8px] font-black uppercase tracking-widest text-slate-500">Portföy</span>
                        </div>
                        <span className={cn(
                            "text-[9px] font-black font-mono px-1 rounded bg-slate-950 border border-white/5", 
                            (summaryData?.change24h || 0) >= 0 ? "text-emerald-400" : "text-rose-400"
                        )}>
                            {(summaryData?.changePercentage || 0) >= 0 ? '+' : ''}{summaryData?.changePercentage.toFixed(2)}%
                        </span>
                    </div>
        ...
        >
            <div className={cn("w-1.5 h-1.5 rounded-full bg-cyan-500 transition-all shadow-[0_0_8px_rgba(6,182,212,0.6)] flex-shrink-0", isOpen && "scale-125")} />
            {isOpen ? <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-white flex-shrink-0" /> : <ChevronLeft className="w-4 h-4 text-slate-400 group-hover:text-white flex-shrink-0" />}
            <span className="[writing-mode:vertical-lr] rotate-180 text-[9px] font-black text-slate-500 uppercase tracking-widest group-hover:text-cyan-400 transition-colors whitespace-nowrap">SMART</span>
        </button>

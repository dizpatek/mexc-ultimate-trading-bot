...
export const PricePredictionWidget = ({ selectedSymbol, onSymbolSelect, onAssetDataUpdate }: PricePredictionWidgetProps) => {
    ...
    const fetchPrediction = useCallback(async () => {
        ...
        setError(null);
        try {
            // Use Matrix V5 Engine (Server-side) for prediction
            ...
            if (res.data && !res.data.error) {
                ...
                setError('Matrix V5 Verisi Alınamadı');
            }
        } catch (error: unknown) {
                    ...
                    <div className="flex flex-col gap-5 h-full">
                        ...
                        <div className="flex items-center justify-between">
                            <h2 className="text-2xl font-black text-white tracking-tight">{symbol.replace('USDT','')}</h2>
                             {/* Trend Badge */}
                            <div className={cn(
                                "flex items-center gap-1.5 px-3 py-1.5 rounded-lg border transition-all shadow-lg",

  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 w-full px-2 py-1.5 sm:py-0">
      ...
      <div className="flex flex-row items-center justify-between sm:justify-end w-full sm:w-auto flex-shrink-0 gap-2 sm:gap-3 pt-1.5 sm:pt-0 border-t sm:border-t-0 border-slate-800/50">
        ...
        <button onClick={focusOnPrices} className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-900/40 border border-slate-800/50 text-[8px] font-black text-cyan-400 hover:bg-cyan-400/10 transition-all group/focus"><Target className="w-2.5 h-2.5" /> ODAKLA</button>
        {onToggleChart && (
          <button onClick={onToggleChart} className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-900/40 border border-slate-800/50 text-[8px] font-black text-violet-400 hover:bg-violet-400/10 transition-all" title={showChart ? "Grafiği Gizle" : "Grafiği Göster"}>{showChart ? <EyeOff size={10} /> : <Eye size={10} />} {showChart ? "GİZLE" : "GÖSTER"}</button>
        )}
        {isLoading && (
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-[9px] font-black text-cyan-400 capitalize tracking-tight pointer-events-none">
            <div className="w-2.5 h-2.5 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" /> Senkronizasyon
          </div>
        )}
        {historyLoading && !isLoading && (
          <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/20 text-[9px] font-black text-cyan-400 capitalize tracking-tight pointer-events-none">
            <div className="w-2.5 h-2.5 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin" /> Geçmiş...
          </div>
        )}
      </div>
    </div>

    ...
    <div className="text-center group/status relative">
      {meta.monitorError && !isClosed && (
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-rose-600 text-white text-[9px] px-2 py-1 rounded shadow-2xl z-50 whitespace-nowrap animate-bounce font-black border border-rose-400/50 flex items-center gap-1.5 min-w-[150px] justify-center">
          ...
          <span>
            {meta.monitorError === "VOLATILITY_GAP_PROTECTION"
              ? "OYNADAKLIK KORUMASI (BEKLE)"
              : `HATA: ${meta.monitorError.toUpperCase()}`}
          </span>
        </div>
      )}
      <div
        className={cn(
          ...
          meta.monitorError && !isClosed
            ...
            : isClosed
              ? "border-white/10 bg-white/5 text-slate-500 animate-none opacity-50"
              : statusColor === "text-emerald-400"
                ? "border-emerald-500/20 bg-emerald-500/5 text-emerald-400 animate-pulse"
      ...
      >
        <span className="opacity-50 text-[7px] mb-0.5">
          {isClosed
            ? "ARŞİVLENMİŞ İŞLEM VERİSİ"
            : liveData
              ? `${timeframe.toUpperCase()} CANLI SİNYAL`
              : "YZ ALIM-SATIM YAKLAŞIMI"}
        </span>
        {isClosed
          ? side === "SELL"
            ? "SATIŞ TAMAM"

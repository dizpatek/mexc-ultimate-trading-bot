  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      if (Array.isArray(data)) {
        const formattedLogs: LogEntry[] = data.map(
          (sig: {
            id: string;
            symbol: string;
            ...
            if (!isSystem && sig.detail) {
              try {
                ...
                if (typeof raw === "object") {
                  extractedDetail = (raw as Record<string, unknown>)?.detail
                    ? String((raw as Record<string, unknown>).detail)
                    : undefined;
                } else if (typeof raw === "string" && raw.startsWith("{")) {
                  const parsed = JSON.parse(raw);
                  extractedDetail = parsed?.detail
                    ? String(parsed.detail)
                    : undefined;
                } else {
                  extractedDetail = sig.executed
            ...
            return {
              ...
              sentiment:
                sig.type === "BUY" ||
                sig.type === "F4_CONFIRMED_BUY" ||
                sig.type === "F4_EARLY_BUY"
                  ? "POSITIVE"
                  : sig.type === "SELL" ||
                      sig.type === "F4_CONFIRMED_SELL" ||
                      sig.type === "F4_EARLY_SELL"
                    ? "NEGATIVE"
                    : "NEUTRAL",

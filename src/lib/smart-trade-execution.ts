...
export async function executeEntry(trade: ExecutionTrade, currentPrice: number, reason: string) {
    ...
    try {
        ...
        if (result?.cummulativeQuoteQty && result?.executedQty && parseFloat(result.executedQty as string) > 0) {
            avgPrice = parseFloat(result.cummulativeQuoteQty as string) / parseFloat(result.executedQty as string);
        }
...
export async function executeExit(trade: ExecutionTrade, currentPrice: number, reason: string, meta: Record<string, unknown>, currentQty: number) {
    ...
    try {
        ...
        if (result?.cummulativeQuoteQty && result?.executedQty && parseFloat(result.executedQty as string) > 0) {
            ...
            realExitPrice = parseFloat(result.cummulativeQuoteQty as string) / executedQty;
        }
...
export async function executePartialTP(trade: ExecutionTrade, currentPrice: number, currentQty: number, exec: { qty: number; targetIndex: number }, meta: Record<string, unknown>, metaUpdates: Record<string, unknown>, tpTriggered: boolean) {
    ...
    try {
        ...
        const res: OrderResult | undefined = isLong ? await marketSellByQty(trade.user_id, trade.symbol, qtyStr) 
                           : await marketBuyByQuote(trade.user_id, trade.symbol, (exec.qty * currentPrice).toFixed(6));
        const executed = parseFloat((res?.executedQty as string) || String(exec.qty));
        newQty -= executed;
...
export async function saveTradeUpdate(id: number, qty: number, meta: Record<string, unknown>) {
    await sql`
        UPDATE orders 
        SET qty = ${qty}, 

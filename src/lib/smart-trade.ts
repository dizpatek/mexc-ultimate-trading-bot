...
export async function handleSmartTrade(
  payload: SmartTradePayload,
  forcedMode?: TradingMode,
) {
  console.log(
    "[SmartTrade] New Request Payload:",
  ...
  console.log(
    `[SmartTrade] Starting ${mode} for ${pair} | Qty: ${qty} | UseExisting: ${!!useExisting}`,
  );
  ...
  try {
    ...
    if (useExisting && mode === "TRADE") {
      ...
      console.log(
        "[SmartTrade] Trailing Entry enabled. Setting order to PENDING.",
      );
      ...
      if (quoteAmt > MAX_QUOTE) {
        console.warn(
          `[SmartTrade] quoteAmt $${quoteAmt.toFixed(2)} exceeds $${MAX_QUOTE} cap. Clamping to max.`,
        );
        quoteAmt = MAX_QUOTE;
      ...
      console.log(
        `[SmartTrade] Executing Market Buy (Base: ${qty.toFixed(8)} | Price: ${currentPrice} | Total USDT: ${quoteStr})`,
      );
      ...
      console.log(
        `[SmartTrade] RAW MEXC BUY Response:`,
        JSON.stringify(entryResult, null, 2),
      );
      console.log(
        `[SmartTrade] Key fields → price: ${entryResult?.price}, executedQty: ${entryResult?.executedQty}, cummulativeQuoteQty: ${entryResult?.cummulativeQuoteQty}`,
      );

      // Calculate real average price from response if possible
      if (
        entryResult?.cummulativeQuoteQty &&
        entryResult?.executedQty &&
        parseFloat(entryResult.executedQty) > 0
      ) {
        avgPrice =
          parseFloat(entryResult.cummulativeQuoteQty) /
          parseFloat(entryResult.executedQty);
        console.log(
          `[SmartTrade] ✅ Used cummulativeQuoteQty/executedQty → avgPrice: ${avgPrice}`,
        );
      } else {
        avgPrice = parseFloat(entryResult?.price || "0") || currentPrice;
        console.log(
          `[SmartTrade] ⚠️ Fallback to price field or ticker → avgPrice: ${avgPrice}`,
        );
      }
      ...
      if (!avgPrice || isNaN(avgPrice) || avgPrice <= 0) {
        throw new Error(
          `Could not determine entry price for ${pair}. Please check connectivity or symbol name.`,
        );
      }
      ...
      if (
        ...
        avgPrice =
          parseFloat(entryResult?.price || "0") ||
          (await getPrice(pair).catch(() => 0));
      }
  ...
  console.log(
    `[SmartTrade] Entry SUCCESS! OrderID: ${orderId} | AvgPrice: ${avgPrice}`,
  );
  ...
  try {
    const hasFollowUp =
      (takeProfit && takeProfit.price && parseFloat(takeProfit.price) > 0) ||
    ...
    const qtyToRecord = entryResult?.executedQty
      ? parseFloat(entryResult.executedQty)
      : 0;
    const quoteToRecord = entryResult?.cummulativeQuoteQty
      ? parseFloat(entryResult.cummulativeQuoteQty)
      : 0;

    const recordId = await insertOrder({
      user_id,
      mexc_order_id: (entryResult?.orderId || entryResult?.id || undefined) as
        | string
        | undefined,
      symbol: pair,
    ...
    console.warn(
      "[SmartTrade] DB recording failed (continuing):",
  ...
  /*
    ...
    */
  console.log(
    "[SmartTrade] Native stop orders disabled. System-managed monitoring will handle exit levels.",
  );

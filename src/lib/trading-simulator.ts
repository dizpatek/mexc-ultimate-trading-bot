...
export class TradingSimulator {
  ...
  async executeMarketBuy(
    ...
    if (isInsufficient) {
      ...
      console.warn(
        `[Simulator/FORCE_BUY] ${symbol}: Insufficient ${quoteAsset}. Missing ${diff.toFixed(2)}. Procedding anyway.`,
      );
    }
  ...
  async executeMarketSell(
    ...
    console.log(
      `[Simulator] After Sell - ${baseAsset}: ${baseBalance.free}, ${quoteAsset}: ${quoteBalance.free}`,
    );

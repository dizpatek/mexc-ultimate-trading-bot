...
const DEFAULT_SCAN_SYMBOLS = [
    ...
    'ATOMUSDT', 'OPUSDT', 'ARBUSDT', 'APTUSDT', 'FILUSDT', 
    'NEARUSDT', 'HBARUSDT', 'ETCUSDT', 'AAVEUSDT', 'RENDERUSDT'
];
...
export class SignalScanner {
    static async resolveScanSymbols(userId: number, mode: 'test' | 'production' = 'test'): Promise<string[]> {
        ...
        return Array.from(new Set([...holdingsSymbols, ...DEFAULT_SCAN_SYMBOLS])).slice(0, 60);
    }
    ...
    private static async scanSymbol(
        ...
        interval: string = '1m'
    ): Promise<{ results: ScanResult[], signalsToInsert: StrategySignalInput[] }> {
        ...
        const newSignals = candidates.filter(c => !existingTypes.includes(c.signalType));
        const deduplicatedSignals = candidates.filter(c => existingTypes.includes(c.signalType));
    ...
    private static evaluateSignals(
        ...
        if (result.whaleStatus && result.whaleStatus !== 'NEUTRAL') {
            signals.push({
                signalType: 'WHALE',
                detail: { detail: ` Whale: ${result.whaleSignalText || result.whaleStatus}`, whaleStatus: result.whaleStatus }
            });
        }
        ...
        if (result.f4EarlyBuy || result.f4ConfirmedBuy) {
            ...
            signals.push({
                signalType: type,
                detail: { detail: ` F4 ${type.replace(/_/g, ' ').replace('F4 ', '')}: Slope=${result.slope?.toFixed(4)} | Accel=${result.acceleration?.toFixed(4)}` }
            });
        }
